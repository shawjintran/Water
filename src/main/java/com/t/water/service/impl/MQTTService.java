...
import com.t.water.config.MQTTConfig;
import com.t.water.config.MQTTConnect;
import com.t.water.utils.PushCallback;
...
public class MQTTService {
	...
	public MQTTService() {
		log.info("MQTT-Server Online");
		this.subscribe("water/sensor",2);
	}
	...
	public void subscribeConnect() {
		try {
			...
			if (subsribeClient==null) {
				...
				subsribeClient.setCallback(new PushCallback(MQTTService.this));
			}
			MqttConnectOptions options = mqttConnect.getOptions();
			//判断拦截状态，这里注意一下，如果没有这个判断，是非常坑的
